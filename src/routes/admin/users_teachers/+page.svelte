<script lang="ts">
	...
	let new_user_forename = "";
	let new_user_surname = "";
	let new_user_gender = "";
	let new_user_email = "";

	function add_user() {
		users.push({
			forename: new_user_forename,
		...
		new_user_forename = new_user_surname = new_user_email = new_user_gender = "";
	}
...
<div class="m-5">
	...
	<form on:submit|preventDefault={add_user}>
		<div
		...
		>
			...
			<div class="col-span-2 sm:col-span-1 sm:place-self-center">
				<input
					type="submit"
					value="Hinzufügen"
					class="w-full rounded-xl bg-white p-3 text-slate-900 hover:bg-blue-200"
				/>
			</div>
		</div>
	</form>
		...
		{#if users.length > 0}
			{#each users as user, i}
				...
				>
					...
					<div class="col-span-2 sm:col-span-1 sm:place-self-center">
						<button
							class="w-full rounded-xl bg-white p-3 text-slate-900 hover:bg-red-200"
			...
			{/each}
		{:else}
			<h1 class="m-8 text-center text-white">Es sind noch keine User vorhanden.</h1>
		{/if}
		<input
